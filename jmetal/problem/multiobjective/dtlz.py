...
class DTLZ7(FloatProblem):
    ...
    def evaluate(self, solution: FloatSolution):
        k = self.number_of_variables - self.number_of_objectives + 1
        g = 1.0 + (9.0 * sum(solution.variables[self.number_of_variables - k:])) / k
        h = self.number_of_objectives - sum(
            [x / (1.0 + g) * (1.0 + sin(3.0 * pi * x)) for x in solution.variables[:self.number_of_objectives - 1]])
